...
func TestResolveBubblewrapMatchesEveryManager(t *testing.T) {
	for _, manager := range bubblewrapManagers() {
		...
		plan := resolve("linux", "", false, lookPathPresent(present))
...
func TestResolveBubblewrapSkipsSudoForRootAndForAHostWithoutSudo(t *testing.T) {
	...
	rootNoSudo := resolve("linux", "", true, lookPathPresent(map[string]bool{"apt-get": true}))
...
func TestResolveBubblewrapNamesARootShellForAUserWithoutSudo(t *testing.T) {
	plan := resolve("linux", "", false, lookPathPresent(map[string]bool{"apk": true}))
	...
	nix := resolve("linux", "", false, lookPathPresent(map[string]bool{"nix": true}))
...
func TestResolveBubblewrapSyncsXbpsAndEnablesNixFlakes(t *testing.T) {
	xbps := resolve("linux", "", true, lookPathPresent(map[string]bool{"xbps-install": true}))
	if got := strings.Join(xbps.Argv, " "); got != "xbps-install -S -y bubblewrap" {
		t.Fatalf("xbps argv = %q", got)
	}
	nix := resolve("linux", "", true, lookPathPresent(map[string]bool{"nix": true}))
...
func TestResolveBubblewrapPrefersDnfOverYum(t *testing.T) {
	plan := resolve("linux", "", false, lookPathPresent(map[string]bool{"dnf": true, "yum": true, "microdnf": true, "sudo": true}))
...
func TestPropertyBubblewrapPlanPicksTheEarliestUnblockedManager(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		plan := resolve("linux", "", root, lookPathPresent(present))
...
// resolve is ResolveBubblewrap without a probe: bwrap counts when lookPath
// finds it or BWRAP_BIN is set, as before the ownership rule.
func resolve(goos, bwrapBin string, root bool, lookPath func(string) (string, error)) BubblewrapPlan {
	return ResolveBubblewrap(BwrapHost{GOOS: goos, BwrapBin: bwrapBin, Root: root, LookPath: lookPath})
}
