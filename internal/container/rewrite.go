...
// RewriteLocalhost replaces localhost / 127.0.0.1 URL authorities with the
// selected runtime's host gateway so a service reachable on the host stays
// reachable from inside the container. It returns (rewritten, changed).
//
...
func RewriteLocalhost(value, gateway string) (string, bool) {
	gateway = strings.TrimSpace(gateway)
	if gateway == "" || !localhostPattern.MatchString(value) {
		return value, false
	}
	return localhostPattern.ReplaceAllString(value, `${1}`+gateway+`${3}`), true
}
