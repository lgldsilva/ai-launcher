...
func TestLiveUpstreamScenarios(t *testing.T) {
	...
	t.Run("ai-memory accepts no-autowire and the 2.4 harness names", func(t *testing.T) {
		// Clap lists canonical names only. Aliases are still accepted, which
		// is the check that matters: the error for an unknown name must name
		// opencode2 and must not call --no-autowire unexpected, and each alias
		// must get past parsing. A dead server stops the run before any agent
		// starts, so this does not open a workstream.
