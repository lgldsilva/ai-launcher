...
| I8 | `COVER_PKGS` does not exist; three divergent exclusion lists | medium | 3 | done (pre-push hook verified locally to report the same number as `make test-coverage`) |
...
| S7 | Hardening and cleanup cluster (12 sub-items) | low | 3 | done |
...
in `--dry-run`). **All three phases are complete.**
...
| S2 | The automatic `--no-hide-config` for a symlinked `.ai-jail` now prints an explicit warning naming the file and the effect (asserted by `TestSymlinkedProjectJailConfigWarnsWhenDisablingHideConfig`). Auto-mounted symlink targets are announced too, instead of appearing only in the final argv echo | restricting the automatic path to symlinks resolving inside the checkout (the item lists it as optional) |
...
| S1 | `preferReleaseInstall` routes any recipe with release assets to the checksum-verified path; `source_url` is a fallback for recipes that publish none. The source path now requires an explicit `allow_unverified: true` in the recipe (`Agent.AllowUnverified` / `Tool.AllowUnverified`) and refuses otherwise, honoring invariant 4; the default catalog declares no `source_url` at all — the ai-memory wrapper recipe's dead `source_url` entry was removed | nothing |
...
| S7 | All sub-items shipped. (a) the explicit `jail_flags` value wins over the permission for the same capability (already done). (b) already fixed by F2's tri-state model — `jail_flags.gpu: true` emits `--gpu` (contract scenario "Emits positive forms for jail toggles forced on"); no code change needed. (c) pre-flight warns `allow-tcp-ports-without-lockdown` when ports are configured without `lockdown`. (d) `AI_MEMORY_NATIVE_BIN` requires the executable bit (regular-file check only on Windows). (e) `checksumFor` requires a filename-anchored match — the lone-hash and `key=<sha256>` fallbacks are gone, the BSD `SHA256 (name) = hash` form stays supported and anchored. (f) `AI_LAUNCHER_UPDATE_TOKEN` is only sent to the default release host; an env-overridden endpoint drops it with a warning. (g) pre-flight warns `catalog-flag-param` naming every enabled `takes_value: false` catalog flag (lighter option chosen over an allowlist; recorded in `docs/design-decisions.md`). (h) the dead `ConstrainForHost` no-op is removed. (i) `replace_other.go` resolves through PATH with the same `resolve %q` error as the unix version and documents its run-and-wait lifecycle. (j) the executor drains the PTY to `io.Discard` when `out == nil` (a chatty child no longer deadlocks `cmd.Wait`) and documents the stdin-copy goroutine lifecycle. (k) the `gh` mount uses `filepath.Join`, fails closed (omitted) when no home is known — contract scenario updated in the same commit, closing the F3 decision — and is deduped against configured mounts, with `coveredByMounts` expanding a leading `~`. (l) a corrupt local config warns and continues with safe defaults, matching the global path. (m) the README TUI preview no longer shows `--exec`, and ARCHITECTURE invariant 11 documents `--exec` and its `len(args) > 0` trigger rule. (n) already done: the root `.ai-jail` is untracked and listed in `.gitignore` | the TOCTOU noted in S7.d is accepted (the managed bin directory is user-writable by design; the check is best-effort and the wrapper fails visibly) |
| I8 | The boundary is stated once per mechanism and the three agree: the `COVERAGE_EXCLUDE` regex in `.ai-standards.env` (anchored as `(^|/)cmd/` so it matches the full module paths in the cover profile — `github.com/lgldsilva/ai-launcher/cmd/...` — without relying on `internal/cmd/` alone), the Makefile `-coverpkg` + awk filter, and `sonar.coverage.exclusions` (now including `internal/selfupdate/**` and `internal/cmd/**`). CI runs `make test-coverage` instead of duplicating the command, and the four stale `COVER_PKGS` references now describe the regex mechanism. Verified locally with the exact pre-push pipeline (`go test ./... -race -coverprofile` + `grep -vE "$COVERAGE_EXCLUDE"` + `go tool cover -func`): the hook-scoped total matches the `make test-coverage` total on the same test set, while the previous `^cmd/` anchor let all 308 `cmd/ai-launcher` blocks into the denominator and diluted the total by ~9 points | nothing |
| F1 | `--fresh` is a CLI flag, an `Options` field, a TUI toggle (rendered only while ai-memory is enabled), and a wrapper flag emitted before the harness token. Pre-flight rejects `--fresh` combined with `--continue` (`fresh-with-continue`). Covered by unit tests, a build scenario and a validation scenario in the contract, and the README options table | nothing |
